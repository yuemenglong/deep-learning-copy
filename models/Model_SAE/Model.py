...
class SAEModel(ModelBase):
    ...
    def onInitializeOptions(self, is_first_run, ask_override):
        ...
        if is_first_run:
            ...
            self.options['learn_mask'] = io.input_bool(
                f"Learn mask? (y/n, ?:help skip:{yn_str[default_learn_mask]} ) : ", default_learn_mask,
        ...
        if is_first_run:
            ...
            self.options['ca_weights'] = io.input_bool(
                f"Use CA weights? (y/n, ?:help skip:{yn_str[def_ca_weights]} ) : ", def_ca_weights,
        ...
        if is_first_run or ask_override:
            ...
            self.options['pixel_loss'] = io.input_bool(
                f"Use pixel loss? (y/n, ?:help skip:{yn_str[def_pixel_loss]} ) : ", def_pixel_loss,
            ...
            self.options['apply_random_ct'] = io.input_bool(
                f"Apply random color transfer to src faceset? (y/n, ?:help skip:{yn_str[default_apply_random_ct]}) : ",
            ...
            if nnlib.device.backend != 'plaidML':  # todo https://github.com/plaidml/plaidml/issues/301
                ...
                self.options['clipgrad'] = io.input_bool(
                    f"Enable gradient clipping? (y/n, ?:help skip:{yn_str[default_clipgrad]}) : ", default_clipgrad,
    ...
    def onInitialize(self):
                ...
                def upscale(dim):
                    def func(x):
                        return SubpixelUpscaler()(
                            LeakyReLU(0.1)(Conv2D(dim * 4, kernel_size=3, strides=1, padding='same')(x)))
        ...
        if self.is_training_mode:
            ...
            if not self.options['pixel_loss']:
                src_loss = K.mean(10 * dssim(kernel_size=int(resolution / 11.6), max_value=1.0)(target_src_masked_opt,
            ...
            if bg_style_power != 0:
                if not self.options['pixel_loss']:
                    src_loss += K.mean((10 * bg_style_power) * dssim(kernel_size=int(resolution / 11.6), max_value=1.0)(
                        psd_target_dst_anti_masked, target_dst_anti_masked))
                else:
                    src_loss += K.mean(
                        (50 * bg_style_power) * K.square(psd_target_dst_anti_masked - target_dst_anti_masked))

            if not self.options['pixel_loss']:
                dst_loss = K.mean(10 * dssim(kernel_size=int(resolution / 11.6), max_value=1.0)(target_dst_masked_opt,
                                                                                                pred_dst_dst_masked_opt))
            else:
                dst_loss = K.mean(50 * K.square(target_dst_masked_opt - pred_dst_dst_masked_opt))

            self.src_dst_train = K.function([warped_src, warped_dst, target_src, target_srcm, target_dst, target_dstm],
                                            [src_loss, dst_loss], self.src_dst_opt.get_updates(src_loss + dst_loss,
                                                                                               self.model.src_dst_trainable_weights))

            if self.options['learn_mask']:
                src_mask_loss = K.mean(K.square(target_srcm - pred_src_srcm))
                dst_mask_loss = K.mean(K.square(target_dstm - pred_dst_dstm))
                self.src_dst_mask_train = K.function([warped_src, warped_dst, target_srcm, target_dstm],
                                                     [src_mask_loss, dst_mask_loss],
                                                     self.src_dst_mask_opt.get_updates(src_mask_loss + dst_mask_loss,
                                                                                       self.model.src_dst_mask_trainable_weights))
    ...
    def predictor_func(self, face=None, dummy_predict=False):
        if dummy_predict:
            self.AE_convert(
                [np.zeros((1, self.options['resolution'], self.options['resolution'], 3), dtype=np.float32)])
        else:
            if self.options['learn_mask']:
    ...
    def get_ConverterConfig(self):
        ...
        self.options['resolution'], self.options['resolution'], 3), converters.ConverterConfigMasked(
            face_type=face_type,
            default_mode=1 if self.options['apply_random_ct'] or self.options['face_style_power'] or self.options[
                'bg_style_power'] else 4,
