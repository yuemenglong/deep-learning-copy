...
class MergeSubprocessor(Subprocessor):
    ...
    def __init__(self, is_interactive, merger_session_filepath, predictor_func, predictor_input_shape, merger_config, frames, frames_root_path, output_path, output_mask_path, model_iter):
        ...
        #     if io.input_bool ("Use saved session?", False):
        #         try:
    ...
    def on_clients_initialized(self):
        ...
        if self.is_interactive:
            ...
            self.masked_keys_funcs = {
                    ...
                    'w' : lambda cfg,shift_pressed: cfg.add_erode_mask_modifier(1 if not shift_pressed else 10),
                    's' : lambda cfg,shift_pressed: cfg.add_erode_mask_modifier(-1 if not shift_pressed else -10),
                    'e' : lambda cfg,shift_pressed: cfg.add_blur_mask_modifier(1 if not shift_pressed else 10),
                    'd' : lambda cfg,shift_pressed: cfg.add_blur_mask_modifier(-1 if not shift_pressed else -10),
                    ...
                    'x' : lambda cfg,shift_pressed: cfg.toggle_mask_mode(1 if not shift_pressed else -1),
                    'c' : lambda cfg,shift_pressed: cfg.toggle_color_transfer_mode(1 if not shift_pressed else -1),
